...
def main():
    ...
    # Load training data
    df=pd.read_csv(f"{dir_name}/Dense_data/test/test.csv")
    # print(df.head)
    ids=df['ids'].values
    print(len(ids))
    #ids=ids[:32]
    img_paths=[]
    dir_path=f"{dir_name}/Dense_data/test/images"
    for i in ids:
    ...
    test_labels = lab
    test_ts_data = np.load(f'{dir_name}/Dense_data/test/test.npy').astype(np.float32)
    del df
    del img_paths
    del ids
    del lab
    ...
    state_dict=torch.load(f"{d_n}/16_model.pth")
