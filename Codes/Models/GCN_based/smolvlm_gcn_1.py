...
class GCNModel(nn.Module):
    def __init__(self):
        ...
        self.conv4 = GCNConv(5, 1)
        self.fcLayer= nn.Linear(284, 284)
        self.final_fc = nn.Linear(284, 256)
...
class SmolVLMTSAD(nn.Module):
    ...
    def __init__(
        ...
        self.gcn_model=GCNModel().to('cuda')

    def GCN_forward(self,x,stride=2,node_feature_size=10,forward_k=6):

        stride = 2
        node_feature_size = 10
        num_nodes = (576 - node_feature_size) // stride + 1  # = 284 nodes

        all_node_features = []
        for b in range(x.shape[0]):
            xi = x[b]
            nodes = torch.stack([xi[j:j+node_feature_size] for j in range(0, 576 - node_feature_size + 1, stride)])
            all_node_features.append(nodes)  # [284,10]

        # -----------------------------
        # 4. Build edge_index:
        #    connect i → i+1, i+2, ..., i+6
        #    add self-loops
        # -----------------------------
        forward_k = 6
        edges = []
        ...
        for i in range(num_nodes):
            ...
            edges.append([i, i])
            # forward connections
            for j in range(1, forward_k + 1):
                if i + j < num_nodes:
                    edges.append([i, i + j])

        edge_index = torch.tensor(edges, dtype=torch.long,device='cuda').T  # [2, num_edges]

        # -----------------------------
        # 5. Pass each graph through GCN
        # -----------------------------
        outputs = []

        for b in range(x.shape[0]):
            x_nodes = all_node_features[b]  # [284,10]
            out_nodes = self.gcn_model(x_nodes, edge_index)  # [284,1]
            outputs.append(out_nodes)

        # -----------------------------
        # 6. Stack all batch results
        # -----------------------------
        outputs = torch.stack(outputs, dim=0).to('cuda')
        return outputs
    ...
    def forward(
        ...
        **kwargs
        ...
        logits=self.GCN_forward(features,stride=2,node_feature_size=10,forward_k=6)
        return logits
...
class VLMTrainer:
    ...
    def custom_loss_function(self,model_logits,true_labels):
