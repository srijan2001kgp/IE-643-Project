...
def student_trainer(d_n,ep,best_val,student_model,teacher_model,num_epoch,lrate,warmup_steps,alpha,temp,device):
    ...
    for epoch in range(ep,num_epochs):
        ...
        if val_loss<best_val:
            early_stop=0
            best_val=val_loss
            torch.save({'epoch': epoch,'model_state_dict': student_model.state_dict(),'best_loss': best_val},save_path)
...
teacher_model_path = f'smolvlmTSAD_1_dense/smolvlm_best_1.pth'
...
checkpoint_path='KD_train_1_1_dense/student_best.pth'
checkpoint=torch.load(checkpoint_path,map_location=DEVICE)
student_model.load_state_dict(checkpoint['model_state_dict'])
print("Student model loaded successfully")
student_model.to(DEVICE)
val_best=checkpoint['best_loss']
...
print(f"Resuming from epoch {last_epoch+1} with current best_loss {val_best:.6f}")
d_n=os.path.join(wd,"KD_train_1_1_dense")
os.makedirs(d_n,exist_ok=True)
print("Starting training...")
torch.cuda.empty_cache()
try:
    st_time=time.time()
    train_loss,val_loss=student_trainer(d_n,last_epoch+1,val_best,student_model,teacher_model,NUM_EPOCH,2e-5,WARM_UP_STEP,1,2,DEVICE)
