...
class VLMTrainer:
        ...
        for batch_idx, batch in enumerate(self.train_loader):
            try:
                ...
                if (batch_idx+1)%100==0:
                    print(f'Batch {batch_idx+1} \'loss\': {loss.item():.6f}, \'lr\': {current_lr:.2e}')
    ...
    def train(self,start_epoch,val_best):
...
checkpoint_path=f"smolvlmTSAD_1_dense/smolvlm_best_1.pth"
...
# Load optimizer state (if same architecture)
...
val_best=checkpoint.get('best_loss',0)
print(f"Resuming training from epoch {start_epoch+1}. Current val loss {val_best}\n")
...
try:
    st_time=time.time()
    train_losses,val_losses = trainer.train(start_epoch,val_best)
