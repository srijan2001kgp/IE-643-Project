...
This project is done for the partial fulfillment of credits for the course **Deep Learning: Theory and Practice(IE643)** during Autumn semster 2025-26 at IIT Bombay. The materials for this project including the report, code walkthrough, code demo, poster, etc. are present can be accessed from this [link](https://drive.google.com/drive/folders/122Sl4xa-G1qGfuxYvxS8EsDsL05oR0W_?usp=drive_link).
### Team members
 ...
 * We have retrained the teacher models `SmolVLMTSAD-0.5` and `SmolVLMTSAD-1` for more number of epochs. The codes for training the model remain the same. The codes for inference are given in [Codes/Inference/vlm/vlm_lstm_0.5_infer.py](Codes/Inference/vlm/vlm_lstm_0.5_infer.py) and [Codes/Inference/vlm/vlm_lstm_1_infer.py](Codes/Inference/vlm/vlm_lstm_1_infer.py) respectively. The scores are available in [Codes/Inference/vlm/vlm_lstm_infer_0.5.log](Codes/Inference/vlm/vlm_lstm_infer_0.5.log) and [Codes/Inference/vlm/vlm_lstm_infer_1.log](Codes/Inference/vlm/vlm_lstm_infer_1.log) respectively.
 * We have modified the teacher model for **dense anomaly** prediction.
    * The codes for training the teacher model are in [Codes/Models/vlm_lstm_0.5_dense.py](Codes/Models/vlm_lstm_0.5_dense.py) and [Codes/Models/vlm_lstm_1_dense.py](Codes/Models/vlm_lstm_1_dense.py) for $\alpha=0.5$ and $\alpha=1$  respectively. The codes for inference are given in [Codes/Inference/vlm/vlm_dense_0.5_infer.py](Codes/Inference/vlm/vlm_dense_0.5_infer.py) and [Codes/Inference/vlm/vlm_dense_1_infer.py](Codes/Inference/vlm/vlm_dense_1_infer.py) respectively. The inference scores are in [Codes/Inference/vlm/vlm_dense_infer_0.5.log](Codes/Inference/vlm/vlm_dense_infer_0.5.log) and [Codes/Inference/vlm/vlm_infer_1_dense.log](Codes/Inference/vlm/vlm_infer_1_dense.log) respectively.
    * The codes for training the student model are in [Codes/Models/kd_train_dense_0.5.py](Codes/Models/kd_train_dense_0.5.py) and [Codes/Models/kd_train_dense_1.py](Codes/Models/kd_train_dense_1.py) respectively. The code for inference is in [Codes/Inference/lstm/student_dense_infer.py](Codes/Inference/lstm/student_dense_infer.py). The inference scores are in [Codes/Inference/lstm/kd_1_0.5_dense_infer.log](Codes/Inference/lstm/kd_1_0.5_dense_infer.log)($\lambda=1,\alpha=0.5$) and [Codes/Inference/lstm/kd_1_1_dense_infer.log](Codes/Inference/lstm/kd_1_1_dense_infer.log)($\lambda=1,\alpha=1$).
 * We have used Graph Convolutional Network on top of `SmolVLM` backbone, The codes for the new VLM models are in [Codes/Models/GCN_based](Codes/Models/GCN_based). To run this code you have to install *torch_geometric* package in addition to the packages in [requirement.txt](Codes/Models/requirement.txt). This approach was found to give poor performance results.
